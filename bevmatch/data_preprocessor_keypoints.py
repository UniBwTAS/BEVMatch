...
from .preprocessor_utils import multiview_img_stack_batch  # aus mmdet3d, nicht mit bevmatch/utils.py verwechseln
from .voxelize import VoxelizationByGridShape, dynamic_scatter_3d


@MODELS.register_module()
class Det3DDataPreprocessorKeypoints(DetDataPreprocessor):
    ...
    def __init__(self,
        ...
        super(Det3DDataPreprocessorKeypoints, self).__init__(
            mean=mean,
            std=std,
    ...
    def forward(self,
        ...
        # unify it, to make it work with every item in data (current, other, ...)
        for data_key in data:

            forwarded = self.forwardSingle(data[data_key], training)
            inputs[data_key] = forwarded['inputs']
            data_samples[data_key] = forwarded['data_samples']

        return {'inputs' : inputs, 'data_samples' : data_samples}

#        return {"current" : self.forwardSingle(data['current'], training), 'previous' : self.forwardSingle(data['prev'], training), 'next' : self.forwardSingle(data['next'], training)}



    def forwardSingle(self,
                data: Union[dict, List[dict]],
                training: bool = False) -> Union[dict, List[dict]]:
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        if 'imgs' in inputs:
            ...
            if data_samples is not None:
                ...
                # needed for the construction of masks, which
        ...
        if 'valid' in inputs:
            batch_inputs['valid'] = inputs['valid']
