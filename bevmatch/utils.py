...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            # max_overlaps MUST be a tensor (not None): TransFusionHead.get_targets_single
            # torch.cat's it across decoder layers, and for the JOINT multi-head model many
            # offroad frames legitimately carry zero boxes (filter_empty_gt=False, kept for the
            # seg + keypoint heads). Returning None here crashed the det head on such frames.
            if num_gts == 0:
